...
export class HttpServer<C extends Context = Context> extends Adapter<
  C,
  OneBotBot<C>
> {
    ...
    this.ctx.server.post(path, (ctx) => {
      if (secret) {
        ...
        const sig = createHmac("sha1", secret)
          .update((ctx.request as any).body[Symbol.for("unparsedBody")])
          .digest("hex");
      ...
      bot.logger.debug("[receive] %o", (ctx.request as any).body);
      dispatchSession(bot, (ctx.request as any).body);
    });
  }
}
